...
def do_dask_merge(copyto, nodataval, dst_shape, dst_masks, src_masks, indices, roff, coff, *blocks):
    ...
    for i in range(len(blocks)):
        ...
        if np.isnan(nodataval):
            region_mask = np.isnan(sub_region)
        elif np.issubdtype(sub_region.dtype, np.floating):
            region_mask = np.isclose(sub_region, nodataval)
        else:
            region_mask = sub_region == nodataval
...
def dask_merge(srcs, method='first'):
    ...
    for raster_idx, src in enumerate(srcs):
        ...
        for i, idx in enumerate(np.ndindex(shape_in_blocks)):
            ...
            inds = np.concatenate((np.zeros((len(inds),1)), inds), axis=1)
            inds = inds.astype(int)
            indst = inds.T 
            inds = list(zip(indst[0], indst[1], indst[2]))
